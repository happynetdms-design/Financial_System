...
-- Backend functions use explicit branch authorization; these policies protect direct client access too.
...
using (exists (select 1 from public.branch_members bm where bm.branch_id=tax_periods.branch_id and bm.user_id=auth.uid()));
...
using (exists (select 1 from public.branch_members bm where bm.branch_id=tax_periods.branch_id and bm.user_id=auth.uid()))
with check (exists (select 1 from public.branch_members bm where bm.branch_id=tax_periods.branch_id and bm.user_id=auth.uid()));
...
using (exists (select 1 from public.tax_periods p join public.branch_members bm on bm.branch_id=p.branch_id where p.id=tax_evidence.tax_period_id and bm.user_id=auth.uid()));
...
using (exists (select 1 from public.tax_periods p join public.branch_members bm on bm.branch_id=p.branch_id where p.id=tax_evidence.tax_period_id and bm.user_id=auth.uid()))
with check (exists (select 1 from public.tax_periods p join public.branch_members bm on bm.branch_id=p.branch_id where p.id=tax_evidence.tax_period_id and bm.user_id=auth.uid()));
...
using (exists (select 1 from public.branch_members bm where bm.branch_id=tax_profile.branch_id and bm.user_id=auth.uid()));
...
using (exists (select 1 from public.branch_members bm where bm.branch_id=tax_profile.branch_id and bm.user_id=auth.uid()))
with check (exists (select 1 from public.branch_members bm where bm.branch_id=tax_profile.branch_id and bm.user_id=auth.uid()));
...
using (exists (select 1 from public.branch_members bm where bm.branch_id=tax_compliance_events.branch_id and bm.user_id=auth.uid()));
