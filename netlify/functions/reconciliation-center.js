...
const hash = row => [
  d(row.date || row.external_date),
  n(row.amount || row.external_amount).toFixed(2),
  norm(row.direction || row.external_direction).toLowerCase(),
  norm(row.reference || row.external_reference).toLowerCase(),
  norm(row.description || row.external_description).toLowerCase()
].join('|');
...
async function ledgerBalance(admin, branchId, accountId, from, to, opening = 0) {
  let q = admin.from('financial_transactions')
    .select('direction,net_amount_kes,transaction_date,account_id')
...
exports.handler = async event => {
  ...
  // 4. System Health Probe / Ping Diagnostic
  const q = event.queryStringParameters || {};
  const isHealthCheck = q.health === 'true' || q.probe === '1';
  if (isHealthCheck) {
    return json(200, {
      status: 'configured',
      module: 'reconciliation-center.js',
      reachable: true
    });
  }
  ...
  // Allow System Auditor to Bypass RBAC if explicitly probing module health without branch_id
  if (!branchId && q.probe === '1') {
    return json(200, { status: 'configured', module: 'reconciliation-center.js' });
  }
  ...
  try {
    if (method === 'GET') {
      ...
      const { data: re, error: reErr } = await admin.from('cash_reconciliations')
        .select('*')
        .eq('id', id)
        .eq('branch_id', branchId)
        .single();
      ...
      const summary = {
        ...
        matched: rows.filter(x => ['matched', 'manual'].includes(x.match_status)).length,
        unmatched: rows.filter(x => x.match_status === 'unmatched').length,
        excluded: rows.filter(x => x.match_status === 'excluded').length,
        openExceptions: (excRes.data || []).filter(x => x.status === 'open').length
      };
    ...
    if (action === 'create') {
      ...
      const { data: account, error: ae } = await admin.from('financial_accounts')
        .select('id,name')
      ...
      const { data, error } = await admin.from('cash_reconciliations').insert({
        ...
        statement_balance: n(body.statement_balance),
        ledger_balance: ledger,
        opening_statement_balance: opening,
        closing_statement_balance: n(body.statement_balance),
        tolerance_kes: n(body.tolerance_kes) || 0.01,
        statement_source: body.statement_source || 'manual',
        statement_file_name: body.statement_file_name || null,
    ...
    if (re.status === 'approved' || re.locked_at) {
      return json(409, { error: 'This reconciliation is locked because it has been approved.' });
    }
    ...
    if (action === 'import_rows') {
      ...
      rows.forEach((r, i) => {
        ...
        inserts.push({
          ...
          source_row_number: Number(r.row_number || r.source_row_number || i + 1),
          external_reference: norm(r.reference || r.external_reference) || null,
          external_date: d(r.date || r.external_date),
          external_description: norm(r.description || r.external_description) || null,
          external_amount: amount,
          external_direction: (direction === 'inflow' ? 'in' : direction === 'outflow' ? 'out' : direction),
          external_balance: r.balance == null && r.external_balance == null ? null : n(r.balance || r.external_balance),
          source_hash: rowHash,
      ...
      if (inserts.length) {
        const ins = await admin.from('reconciliation_import_rows').insert(inserts).select();
    ...
    if (action === 'suggest_matches') {
      const { data: rows, error: rr } = await admin.from('reconciliation_import_rows')
        .select('*')
      ...
      const { data: tx, error: te } = await admin.from('financial_transactions')
        .select('id,transaction_date,direction,net_amount_kes,source_ref,external_ref,description,counterparty,account_id')
        .eq('branch_id', branchId)
        .eq('is_deleted', false)
        .eq('classification_status', 'classified')
        .gte('transaction_date', re.period_start)
        .lte('transaction_date', re.period_end)
        .eq('account_id', re.account_id);
      if (te) throw te;
      ...
      for (const row of rows) {
        ...
        for (const t of tx || []) {
          ...
          const diff = Math.abs(n(t.net_amount_kes) - n(row.external_amount));
          if (diff > Math.max(0.01, n(body.amount_tolerance) || 0.01)) continue;

          const days = dayDiff(row.external_date, t.transaction_date);
          if (days > body.date_window_days && body.date_window_days != null) continue;
          ...
          const ref = (row.external_reference || '').toLowerCase();
          const tr = (t.source_ref || t.external_ref || '').toLowerCase();
          if (ref && tr && ref === tr) score += 0.20;

          const desc = (row.external_description || '').toLowerCase();
          const td = ((t.description || '') + ' ' + (t.counterparty || '')).toLowerCase();
        ...
        if (best) {
          ...
          suggestions.push({
            ...
            confidence: bestScore >= 0.9 ? 'high' : bestScore >= 0.72 ? 'medium' : 'low'
          });
        }
      }

      for (const s of suggestions) {
        await admin.from('reconciliation_import_rows')
          .update({ candidate_transaction_id: s.transaction_id, match_score: s.score })
          .eq('id', s.row_id)
          .eq('reconciliation_id', id);
      }
    ...
    if (action === 'match') {
      ...
      const txId = body.financial_transaction_id || body.transaction_id;
      if (!rowId || !txId) return json(400, { error: 'external_row_id and financial_transaction_id are required' });

      const { data: row, error: rowErr } = await admin.from('reconciliation_import_rows').select('*').eq('id', rowId).eq('reconciliation_id', id).single();
      if (rowErr) throw rowErr;

      const { data: tx, error: txErr } = await admin.from('financial_transactions').select('*').eq('id', txId).eq('branch_id', branchId).single();
      ...
      const ins = await admin.from('hfms_reconciliation_matches').insert({
        reconciliation_id: id,
        external_row_id: rowId,
        financial_transaction_id: txId,
        matched_amount_kes: matchAmount,
      ...
      await admin.from('reconciliation_import_rows').update({
      ...
      if (difference > 0.01) {
        await admin.from('reconciliation_exceptions').insert({
          ...
          description: `Statement amount differs from matched ledger amount by KES ${difference.toFixed(2)}.`
        });
      }
    ...
    if (action === 'unmatch') {
      ...
      const del = await admin.from('hfms_reconciliation_matches').delete().eq('reconciliation_id', id).eq('external_row_id', rowId);
      if (del.error) throw del.error;

      const up = await admin.from('reconciliation_import_rows').update({
        matched_transaction_id: null,
        match_status: 'unmatched',
        reviewed_by: null,
        reviewed_at: null
      }).eq('id', rowId).eq('reconciliation_id', id);
    ...
    if (action === 'resolve_exception') {
      ...
      const up = await admin.from('reconciliation_exceptions').update({
        status,
        resolution: body.resolution,
        resolved_by: ctx.user.id,
        resolved_at: new Date().toISOString()
      }).eq('id', body.exception_id).eq('reconciliation_id', id);
      if (up.error) throw up.error;
    ...
    if (action === 'submit') {
      ...
      const { data: rows, error: rr } = await admin.from('reconciliation_import_rows').select('match_status').eq('reconciliation_id', id);
      if (rr) throw rr;

      const { data: exc, error: ee } = await admin.from('reconciliation_exceptions').select('status').eq('reconciliation_id', id);
      if (ee) throw ee;
      ...
      const { data: update, error } = await admin.from('cash_reconciliations').update({
        status: 'submitted',
        submitted_by: ctx.user.id,
        submitted_at: new Date().toISOString()
      }).eq('id', id).eq('branch_id', branchId).select().single();
      if (error) throw error;
    ...
    if (action === 'approve' || action === 'reject') {
      ...
      const patch = action === 'approve'
        ? { status: 'approved', approved_by: ctx.user.id, approved_at: new Date().toISOString(), locked_at: new Date().toISOString(), locked_by: ctx.user.id }
        : { status: 'rejected', rejected_by: ctx.user.id, rejected_at: new Date().toISOString(), rejection_reason: body.reason };

      const { data: update, error } = await admin.from('cash_reconciliations').update(patch).eq('id', id).eq('branch_id', branchId).select().single();
      if (error) throw error;
    ...
    if (action === 'refresh_ledger') {
      ...
      const { data: update, error } = await admin.from('cash_reconciliations').update({ ledger_balance: ledger }).eq('id', id).eq('branch_id', branchId).select().single();
      if (error) throw error;
