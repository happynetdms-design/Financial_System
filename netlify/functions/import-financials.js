  ...
  for(const r of rows){
    try{
      ...
      if(source==='organization_utility'){
        ...
        tx={branch_id:branchId,transaction_date:date,transaction_type:'revenue',direction:'in',
        ...
        // Happynet's approved source-of-truth rule: every successful Tende INCOMING
        // transaction in this feed represents owner funding/loan activity. John is
        // the default owner lender; the raw source is retained for traceability.
        if(isIncoming){
          ...
          tx={branch_id:branchId,transaction_date:date,transaction_type:'owner_loan_funding',direction:'in',
          ...
          tx={branch_id:branchId,transaction_date:date,transaction_type:'expense',direction:'out',
      ...
      // Every posted source transaction also creates a cash movement. This is the bridge
      // between the source ledger and the cash/reconciliation engine.
      const cashMovement={
        branch_id:branchId,
    ...
    }catch(e){result.errors.push({ref:r.source_ref||r.ref||r.receipt_no||'',error:e.message});}
  }
