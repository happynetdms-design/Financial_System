...
async function context(admin, branchId){
  ...
  let revenue=0, expenses=0, funding=0, repayments=0; const byMonth={}; const byCategory={};
  for(const r of tx||[]){ const a=money(r.net_amount_kes), m=(r.transaction_date||'').slice(0,7); byMonth[m]??={revenue:0,expenses:0,funding:0,repayments:0};
  ...
  return {as_of:new Date().toISOString(), branch_id:branchId, ledger:{revenue,expenses,operating_result:revenue-expenses,owner_loan_funding:funding,owner_loan_repayment:repayments,net_financing_adjusted_cash:revenue+funding-expenses-repayments,transaction_count:(tx||[]).length},monthly:byMonth,expense_categories:byCategory,accounts:accounts||[],allocations:alloc||[],alerts:alerts||[],budgets:budgets||[],loans:loans||[],recent_transactions:(tx||[]).slice(0,100)};
}
...
exports.handler = async(event)=>{
  ...
  const admin=adminClient(); let body; try{body=JSON.parse(event.body||'{}')}catch{return json(400,{error:'Invalid JSON.'})}
  ...
  try{
    ...
    const prompt=`LIVE HFMS FINANCIAL CONTEXT:\n${JSON.stringify(financial)}\n\nUSER REQUEST:\n${question}\n\nMODE:${mode}\nReturn a structured answer. If an action is appropriate, include a final ACTION block in JSON on one line: {"action_type":"...","risk_level":"low|medium|high|critical","payload":{...},"confirmation_text":"..."}. Do not claim execution.`;
    const messages=[...history.slice(-10).filter(m=>m&&(m.role==='user'||m.role==='assistant')), {role:'user',content:prompt}];
    const res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'content-type':'application/json','x-api-key':process.env.ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:MODEL,max_tokens:1800,system:SYSTEM,messages})});
    if(!res.ok){console.error(await res.text());return json(502,{error:'AI provider unavailable.'})}
    const data=await res.json(); const answer=extractText(data);
    let proposed_action=null; const match=answer.match(/ACTION\s*:\s*(\{[\s\S]*\})\s*$/i); if(match){try{proposed_action=JSON.parse(match[1])}catch{}}
    if(proposed_action){
      const {data:ar,error}=await admin.from('ai_action_requests').insert({branch_id:branchId,user_id:ctx.user.id,action_type:proposed_action.action_type,action_payload:proposed_action.payload||{},risk_level:proposed_action.risk_level||'medium',status:'awaiting_confirmation',confirmation_text:proposed_action.confirmation_text||'Confirm this action.'}).select().single();
      if(error)console.error('action insert',error); else proposed_action={...proposed_action,id:ar.id,status:'awaiting_confirmation'};
    }
    return json(200,{answer,proposed_action,as_of:financial.as_of});
  }catch(e){console.error(e);return json(500,{error:'Unexpected AI copilot error.'})}
};
