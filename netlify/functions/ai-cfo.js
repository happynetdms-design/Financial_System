...
async function buildContext(db, branchId, userId){
  const [txR, allocR, alertR, budgetR, loanR, acctR, recR, supplierR, insightR, memoryR, cashR] = await Promise.all([
    ...
    db.from('profit_first_allocations').select('*').eq('branch_id',branchId).order('created_at',{ascending:false}).limit(60),
    db.from('financial_alerts').select('alert_type,severity,title,message,status,created_at').eq('branch_id',branchId).in('status',['open','acknowledged']).order('created_at',{ascending:false}).limit(50),
    db.from('budgets').select('*').eq('branch_id',branchId).order('period_start',{ascending:false}).limit(60),
    ...
    db.from('recurring_expenses').select('id,description,amount_kes,frequency,next_due_date,active').eq('branch_id',branchId).order('next_due_date').limit(50),
    db.from('suppliers').select('id,canonical_name,status').eq('branch_id',branchId).order('canonical_name').limit(150),
    db.from('ai_financial_insights').select('classification,title,message,evidence,created_at').eq('branch_id',branchId).order('created_at',{ascending:false}).limit(30),
    db.from('ai_cfo_memory').select('memory_type,content,source,updated_at').eq('branch_id',branchId).eq('user_id',userId).eq('active',true).order('updated_at',{ascending:false}).limit(20),
    db.from('cash_movements').select('account_id,direction,amount_kes').eq('branch_id',branchId).eq('is_deleted',false).limit(2000)
  ]);
  ...
  const cashBalance = (cashR.data || []).reduce((s, r) => {
    ...
    if (r.direction === 'inflow' || r.direction === 'in') return s + amt;
    return s;
  }, 0);
...
exports.handler = async event => {
  ...
  const access = await requireBranchAccess(event, requireUser, db, branchId, {write: false}); 
  ...
  try {
    ...
    if(!conversationId) {
      const {data: c, error: e} = await db.from('ai_conversations').insert({
        branch_id: branchId, 
    ...
    if (body.history?.length) {
      ...
      const { data: dbMessages } = await db.from('ai_messages')
        .select('role,content')
    ...
    if(!process.env.ANTHROPIC_API_KEY){ 
      ...
      if(!response.ok){ 
        ...
        result = fallbackAnswer(context, question); 
        result.answer = 'AI provider was unavailable, so HFMS returned a ledger-grounded deterministic answer.\n\n' + result.answer; 
      } else { 
        const data = await response.json(); 
        const text = extractText(data); 
        result = safeJson(text) || fallbackAnswer(context, question); 
      }
    }
    ...
    await db.from('ai_messages').insert({
      ...
      role: 'assistant',
      content: answer,
      message_type: result.classification || 'chat',
    ...
    if(result.action){
      ...
      const {data: ar, error: e} = await db.from('ai_action_requests').insert({
        branch_id: branchId,
        user_id: access.user.id,
        conversation_id: conversationId,
        ...
        confirmation_text: a.confirmation_text || 'Confirm this proposed action.'
      }).select().single();
    ...
    if(result.classification && ['FACT','CALCULATION','FORECAST','RECOMMENDATION','RISK'].includes(result.classification)){
      await db.from('ai_financial_insights').insert({
        branch_id: branchId,
        insight_type: 'cfo_conversation',
        classification: result.classification,
        title: 'HFMS CFO analysis',
        message: answer,
        evidence: { citations: result.citations || [], conversation_id: conversationId }
      });
    }
    ...
    return json(200, {
      conversation_id: conversationId,
      answer,
      classification: result.classification || 'FACT',
      citations: result.citations || [],
      proposed_action: action,
      as_of: context.generated_at
    });
  } catch(e) { 
    console.error('ai-cfo', e); 
    return json(500, { error: e.message || 'HFMS CFO failed.' }); 
  }
};