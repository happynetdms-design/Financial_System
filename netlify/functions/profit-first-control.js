...
async function revenueFor(admin, branchId, period) {
  ...
  return (data || []).reduce((s, r) => s + n(r.net_amount_kes), 0);
}
...
exports.handler = async event => {
  ...
  if (!branchId && event.queryStringParameters && event.queryStringParameters.probe === '1') {
    return json(200, { status: 'configured', module: 'profit-first.js' });
  }
  ...
  try {
    ...
    if (body.action === 'update_settings') {
      ...
      const vals = {};
      for (const k of Object.values(pctKeys)) {
        if (body[k] !== undefined) vals[k] = n(body[k]);
      }
      const total = BUCKETS.reduce((s, b) => s + n(vals[pctKeys[b]]), 0);
      if (Math.abs(total - 100) > 0.001) return json(400, { error: `Profit First percentages must total 100%. Current total: ${total}%.` });
      ...
      const { data, error } = await admin.from('profit_first_settings').upsert({ branch_id: branchId, ...patch }, { onConflict: 'branch_id' }).select().single();
      if (error) throw error;
    ...
    if (body.action === 'prepare_cycle') {
      ...
      const { data: settings, error: se } = await admin.from('profit_first_settings').select('*').eq('branch_id', branchId).single();
      if (se) throw se;
      const revenue = await revenueFor(admin, branchId, body.period);
      const amounts = {};
      for (const b of BUCKETS) amounts[b] = Math.round(revenue * n(settings[pctKeys[b]]) / 100 * 100) / 100;
      const total = Object.values(amounts).reduce((s, v) => s + v, 0);
      amounts.opex = Math.round((amounts.opex + (revenue - total)) * 100) / 100;
      ...
      const { data, error } = await admin.from('allocations').upsert(rows, { onConflict: 'branch_id,period,bucket' }).select();
      if (error) throw error;
      const { data: cycle, error: ce } = await admin.from('profit_first_cycles').upsert({ branch_id: branchId, period: periodDate(body.period), revenue_kes: revenue, target_pct_total: 100, status: 'prepared', prepared_by: ctx.user.id, prepared_at: new Date().toISOString() }, { onConflict: 'branch_id,period' }).select().single();
      if (ce) throw ce;
      return json(200, { cycle, allocations: data, settings });
    }

    if (body.action === 'request_approval') {
      const period = periodDate(body.period);
      const { data: rows, error } = await admin.from('allocations').select('*').eq('branch_id', branchId).eq('period', period);
      if (error) throw error;
      if (!rows?.length) return json(404, { error: 'No allocation prepared for this period.' });
    ...
    if (body.action === 'approve_cycle') {
      ...
      if (error) throw error;
      for (const a of rows) {
        await admin.from('allocations').update({ approved_by: ctx.user.id, approved_at: new Date().toISOString() }).eq('id', a.id);
        await admin.from('allocation_approvals').upsert({ allocation_id: a.id, branch_id: branchId, status: 'approved', requested_by: ctx.user.id, reviewed_by: ctx.user.id, reviewed_at: new Date().toISOString(), reason: body.reason || null }, { onConflict: 'allocation_id' });
      }
    ...
    if (body.action === 'record_transfer') {
      const { data: a, error: ae } = await admin.from('allocations').select('*').eq('id', body.allocation_id).eq('branch_id', branchId).single();
      if (ae) throw ae;
      if (!a.approved_at) return json(409, { error: 'Allocation must be approved before transfer.' });
      const actual = n(body.actual_amount_kes);
      if (actual <= 0) return json(400, { error: 'Actual transfer amount must be greater than zero.' });
    ...
    if (body.action === 'verify_proof') {
      ...
      const { data: p, error } = await admin.from('allocation_proofs').update({ proof_status: body.status === 'rejected' ? 'rejected' : 'verified', verified_by: ctx.user.id, verified_at: new Date().toISOString(), verification_note: body.note || null }).eq('id', body.proof_id).eq('branch_id', branchId).select().single();
      if (error) throw error;
      const { data: a } = await admin.from('allocations').select('*').eq('id', p.allocation_id).single();
      if (p.proof_status === 'verified') await admin.from('allocations').update({ transfer_status: 'verified' }).eq('id', p.allocation_id);
    ...
    if (body.action === 'close_cycle') {
      ...
      const period = periodDate(body.period);
      const { data: rows, error } = await admin.from('allocations').select('*,allocation_proofs(*)').eq('branch_id', branchId).eq('period', period);
      if (error) throw error;
    ...
    if (body.action === 'score') {
      ...
      const { data: rows, error } = await admin.from('allocations').select('*,allocation_proofs(*)').eq('branch_id', branchId).eq('period', period);
      if (error) throw error;
      let target = 0, transferred = 0, verified = 0, variance = 0;
      for (const a of rows || []) {
        target += n(a.amount_kes);
        transferred += n(a.transferred_amount_kes);
        if (a.transfer_status === 'verified') verified += 1;
        variance += Math.abs(n(a.variance_kes));
      }
      const count = (rows || []).length;
      const completion = count ? verified / count * 100 : 0;
      const transferRate = target ? Math.min(100, transferred / target * 100) : 0;
      const score = Math.round((completion * 0.6 + transferRate * 0.4) * 100) / 100;
    ...
    console.error('profit-first error', e);
    return json(500, { error: e.message || 'Profit First control failed.' });
  }
};