  ...
  try{
    ...
    if(method === 'POST'){
      ...
      const payload = rows.map(r => ({
        // Client generates a real UUID at creation time (crypto.randomUUID())
        // and keeps using it — no server round trip needed before the UI can
        // reference the record again. Omit to let Postgres default it.
        ...(r.id ? { id: r.id } : {}),
        branch_id: branchId,
