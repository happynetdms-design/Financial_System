...
async function ensureRules(admin, branchId) {
  ...
  for (const [key, name, severity, lead] of defaults) {
    if (!map.has(key)) {
      await admin.from('hfms_automation_rules').insert({
        branch_id: branchId,
        rule_key: key,
        name,
        description: name,
...
async function queueNotification(admin, branchId, rule, eventKey, message, payload = {}) {
  ...
  let recipient = null;
  if (channel === 'email') {
    recipient = process.env.HFMS_ALERT_EMAIL || null;
  } else if (channel === 'sms') {
    recipient = process.env.HFMS_ALERT_SMS || null;
  }
  ...
  for (const row of rows || []) {
    ...
    try {
      if (row.channel === 'in_app') {
        ...
        if (!process.env.RESEND_API_KEY || !row.recipient) {
          throw new Error('Email provider not configured. Set RESEND_API_KEY and HFMS_ALERT_EMAIL.');
        }
        ...
        const resp = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: process.env.HFMS_FROM_EMAIL || 'Happynet HFMS <onboarding@resend.dev>',
            to: [row.recipient],
            subject: row.subject,
            text: row.body
          })
        ...
        if (!resp.ok) {
          throw new Error(`Email provider returned ${resp.status}: ${await resp.text()}`);
        }
        ...
        if (!url) {
          throw new Error(`No ${row.channel} provider configured.`);
        }
        ...
        const resp = await fetch(url, {
          ...
          body: JSON.stringify({
            ...
            message: row.body,
            payload: row.payload,
            source: 'Happynet HFMS'
          })
        });

        if (!resp.ok) {
          throw new Error(`Notification provider returned ${resp.status}`);
        }
  ...
  for (const item of items || []) {
    ...
    const tx = await admin
      ...
      .insert({
        ...
        charges_kes: 0,
        net_amount_kes: amount,
        account_id: null,
        category_id: item.category_id,
        expense_id: exp.data.id,
        source_system: 'recurring',
        source_ref: ref,
        counterparty: item.supplier_id,
        description: item.description,
    ...
    if (tx.error) {
      await admin.from('expenses').delete().eq('id', exp.data.id);
      continue;
    }
    ...
    const cm = await admin.from('cash_movements').insert({
      ...
      to_account_id: null,
      financial_transaction_id: tx.data.id,
      source_ref: ref,
      description: item.description,
      reason: 'HFMS automated recurring expense',
      created_by: null
    });

    if (cm.error) {
      /* ledger remains traceable; flag for reconciliation rather than delete */
    }

    await admin.from('recurring_expense_runs').insert({
    ...
    await admin
      .from('recurring_expenses')
      .update({
        next_due_date: nextDue(item.next_due_date, item.frequency),
        updated_at: new Date().toISOString()
      })
      .eq('id', item.id);
...
async function scanBranch(admin, branch, runId) {
  ...
  const [txR, cashR, allocR, budgetR, taxR, anomR, reconR, targetR] = await Promise.all([
    admin
      .from('financial_transactions')
      .select('transaction_date,transaction_type,direction,net_amount_kes')
      .eq('branch_id', branchId)
      .eq('is_deleted', false)
    ...
    admin
      .from('cash_movements')
      .select('direction,amount_kes')
      .eq('branch_id', branchId)
      .eq('is_deleted', false)
      .limit(20000),
    admin
      .from('profit_first_allocations')
      .select('id,status,expected_amount_kes,period')
      .eq('branch_id', branchId)
      .order('period', { ascending: false })
      .limit(50),
    admin
      .from('budgets')
      .select('period,budget_kes')
      .eq('branch_id', branchId)
      .gte('period', start)
      .lte('period', end),
    admin
      .from('tax_periods')
      .select('id,payment_due_date,filing_due_date,amount_due_kes,amount_paid_kes,payment_status,filing_status')
      .eq('branch_id', branchId)
      .limit(200),
    admin
      .from('anomaly_events')
      .select('id,status,severity,message')
      .eq('branch_id', branchId)
      .eq('status', 'open')
      .limit(50),
    admin
  ...
  const tx = txR.data || [];
  const cashRows = cashR.data || [];

  let rev = 0;
  let exp = 0;
  for (const t of tx) {
    const a = n(t.net_amount_kes);
    if (t.transaction_type === 'revenue' && t.direction === 'in') rev += a;
    if (t.transaction_type === 'expense' && t.direction === 'out') exp += a;
  }

  let cash = 0;
  for (const c of cashRows) {
    cash += c.direction === 'outflow' ? -n(c.amount_kes) : n(c.amount_kes);
  }

  const targets = targetR.data || [];
  const minCash = n((targets.find(x => x.metric_key === 'min_cash') || {}).target_value);
  ...
  const add = async (key, msg, obs, thr, payload = {}) => {
    ...
    if (was) {
      created += (await queueNotification(admin, branchId, rule, evKey, msg, payload)) ? 1 : 0;
    }
  };
  ...
  if (minCash > 0 && cash < minCash) {
    await add(
      'low_cash',
      `Available cash is KES ${cash.toLocaleString()} versus the minimum target of KES ${minCash.toLocaleString()}.`,
      cash,
      minCash,
      { cash, minCash }
    );
  ...
  if (budget > 0 && exp > budget) {
    await add(
      'budget_overrun',
      `Current-month expenses are KES ${(exp - budget).toLocaleString()} above the configured budget.`,
      exp,
      budget,
      { expenses: exp, budget }
    );
  ...
  if (rev > 0 && exp > rev) {
    await add(
      'negative_result',
      `Current-month expenses exceed revenue by KES ${(exp - rev).toLocaleString()}.`,
      exp,
      rev,
  ...
  if (open.length) {
    await add(
      'profit_first_missing',
      `${open.length} Profit First allocation cycle(s) still require completion.`,
      open.length,
      0,
      { allocations: open }
    );
  ...
  const now = new Date();
  const lead = ruleMap.get('tax_due')?.lead_days ?? 30;

  for (const p of taxR.data || []) {
    const due = p.payment_due_date || p.filing_due_date;
    if (!due) continue;
    ...
    if (days < 0 && ruleMap.get('tax_overdue')?.enabled) {
      await add(
        'tax_overdue',
        `Tax period ${p.id} is overdue by ${Math.abs(days)} day(s) with KES ${outstanding.toLocaleString()} outstanding.`,
        outstanding,
        0,
        { tax_period_id: p.id, days_overdue: Math.abs(days) }
      );
    } else if (days >= 0 && days <= lead) {
      await add(
        'tax_due',
        `Tax payment/filing is due in ${days} day(s) with KES ${outstanding.toLocaleString()} outstanding.`,
        outstanding,
        0,
        { tax_period_id: p.id, days_until_due: days }
      );
    }
  }

  if ((anomR.data || []).length) {
    await add(
      'anomaly_open',
      `${anomR.data.length} unresolved financial anomaly event(s) require review.`,
      anomR.data.length,
      0,
  ...
  if ((reconR.data || []).length) {
    await add(
      'reconciliation_pending',
      `${reconR.data.length} cash reconciliation(s) remain open or pending approval.`,
      reconR.data.length,
      0,
      { reconciliations: reconR.data }
    );
  }

  const { data: due } = await admin
    .from('recurring_expenses')
    .select('id,description,amount_kes,next_due_date')
    .eq('branch_id', branchId)
    .eq('active', true)
    .lte('next_due_date', today());
  ...
  if ((due || []).length) {
    await add(
      'recurring_due',
      `${due.length} recurring expense(s) are due for controlled posting/review.`,
      due.length,
      0,
      { recurring: due }
    );

    const rule = ruleMap.get('recurring_due');
    if (rule?.auto_execute) {
      const r = await postDueRecurring(admin, branchId, rule, runId);
      prepared += r.prepared;
      executed += r.executed;
    }
  }
...
async function runAutomation(trigger = 'scheduled', branchId = null) {
  ...
  try {
    let q = admin.from('branches').select('id,name').eq('is_active', true);
    ...
    for (const b of branches || []) {
      const r = await scanBranch(admin, b, runId);
      totals.branches++;
      totals.rules += r.rules;
      totals.evaluated += r.evaluated;
      totals.created += r.created;
      totals.prepared += r.prepared;
    ...
    await admin
      ...
      .update({
        ...
        branches_scanned: totals.branches,
        rules_evaluated: totals.evaluated,
        notifications_created: totals.created,
        notifications_sent: delivery.sent,
        actions_prepared: totals.prepared,
        actions_executed: totals.executed,
        summary: { ...totals, delivery }
      })
      .eq('id', runId);
...
exports.handler = async event => {
  ...
  if (method === 'POST') {
    ...
    if (branchId) {
      const ctx = await requireBranchAccess(event, requireUser, adminClient(), branchId, { write: true });
      if (ctx.error) {
  ...
  try {
    return json(200, await runAutomation(trigger, branchId));
  } catch (e) {
