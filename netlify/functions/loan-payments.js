  ...
  try{
    if(method === 'GET'){
      ...
      const { data, error } = await admin
        .from('loan_payments').select('*').in('loan_id', ids).eq('is_deleted',false)
    ...
    if(method === 'POST'){
      ...
      const { data: loan } = await admin.from('loans').select('id,branch_id').eq('id', body.loan_id).eq('branch_id', branchId).maybeSingle();
      ...
      if(error) return json(500, { error: error.message });
      // Mirror the repayment into the unified financial ledger. This keeps loan reporting and cash reporting aligned.
      const tx = await admin.from('financial_transactions').insert({branch_id:branchId,transaction_date:body.payment_date,transaction_type:'owner_loan_repayment',direction:'outflow',gross_amount_kes:body.amount_kes,charges_kes:0,net_amount_kes:body.amount_kes,loan_id:body.loan_id,source_system:'loan_payment',source_ref:data.id,description:body.note||'Owner loan repayment',created_by:ctx.user.id}).select().maybeSingle();
      if(tx.error && tx.error.code!=='23505') return json(500,{error:tx.error.message});
    ...
    if(method === 'PATCH'){
      ...
      const { data, error } = await admin.from('loan_payments').update(patch).eq('id', body.id).eq('is_deleted',false).select().maybeSingle();
      if(error) return json(500, { error: error.message });
    ...
    if(method === 'DELETE'){
      if(!body.id) return json(400, { error: 'id is required.' });
      const { error } = await admin.from('loan_payments').update({is_deleted:true}).eq('id', body.id).eq('is_deleted',false);
      if(error) return json(500, { error: error.message });
