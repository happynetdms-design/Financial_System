...
function viewAssistant(){
  ...
  return `
    ...
    <div class="card" style="max-width:980px;">
      ...
      <div id="assistant-thread" style="display:flex; flex-direction:column; gap:12px; min-height:180px; max-height:55vh; overflow:auto; margin-bottom:14px;">
        ...
        ${assistantMessages.map(m => `
          <div style="align-self:${m.role==='user'?'flex-end':'flex-start'}; max-width:90%;">
            <div style="font-size:11px; color:var(--muted); margin-bottom:2px; text-transform:uppercase; letter-spacing:.04em;">${m.role==='user'?'You':'HFMS CFO'}${m.classification?` · ${m.classification}`:''}</div>
            ...
            ${m.citations?.length?`<div class="hint" style="margin-top:5px;">Evidence: ${m.citations.map(c=>`${String(c.source||'HFMS').replace(/</g,'&lt;')} — ${String(c.detail||'').replace(/</g,'&lt;')}`).join(' · ')}</div>`:''}${m.action?`<div style="margin-top:8px;"><button class="btn gold sm" data-cfo-confirm="${m.action.id}">Confirm action</button> <button class="btn ghost sm" data-cfo-cancel="${m.action.id}">Cancel</button></div>`:''}
          </div>`).join('')}
      ...
      ${assistantConversationId?`<div class="hint" style="margin-top:8px;">Conversation saved in HFMS · ${assistantConversationId.slice(0,8)}…</div>`:''}
    </div>`;
}
...
function readOnlyNotice(){
  return `<div class="hint" style="display:flex; align-items:center; gap:8px; padding:12px 14px; background:#EFE9D9; border-radius:10px; margin-bottom:16px;">${ic('lock',14)}Your role (${(state.role||'').replace(/_/g,' ')}) has read-only access here.</div>`;
}
...
async function loadControlCenter(){
  ...
  el.innerHTML=`
    <div class="grid kpi">
      ...
      <div class="card kpi"><div class="kpi-head"><h3>John Loan Funding</h3><div class="kpi-icon tone-gold">${ic('landmark',16)}</div></div><div class="big">${KES(p.ownerLoanFunding)}</div><div class="foot">Funding — not revenue</div></div>
    </div>
    <div class="form-card" style="margin-top:18px"><div class="section-head"><h2>Financial intelligence</h2><span class="tag neutral">Prediction</span></div><div id="hfms-forecast">Calculating forecast…</div></div>
    <div class="grid two-col" style="margin-top:18px">
      <div class="form-card"><div class="section-head"><h2>Cash & reconciliation</h2></div><div id="hfms-cash-list">Loading accounts…</div><div class="section-head" style="margin-top:18px"><h3>Transfer between accounts</h3></div><div class="form-row"><div><label>From</label><select id="cash-from"></select></div><div><label>To</label><select id="cash-to"></select></div></div><div class="form-row"><div><label>Amount (KES)</label><input id="cash-transfer-amount" type="number" min="0" step="0.01"></div><div><label>Reason</label><input id="cash-transfer-reason" placeholder="e.g. Move tax reserve"></div></div><button class="btn gold" id="btn-cash-transfer">Record transfer</button></div>
...
function viewControlCenter(){return `<div class="topbar"><div><h1>Financial Control Center</h1><div class="sub">Production financial position, cash controls, reconciliation and automated checks.</div></div></div><div id="hfms-control-content"><div class="form-card">Loading financial control center…</div></div>`;}
...
async function loadManagementReport(){
  ...
  el.innerHTML=`<div class="report-sheet" id="printable-financial-report"><div class="section-head"><h2>Happynet Financial Management Report</h2><span class="tag neutral">${d.period.from} → ${d.period.to}</span></div><div class="grid kpi"><div class="card kpi"><h3>Revenue</h3><div class="big">${KES(s.revenue)}</div></div><div class="card kpi"><h3>Expenses</h3><div class="big">${KES(s.expenses)}</div></div><div class="card kpi"><h3>Net Profit</h3><div class="big">${KES(s.netProfit)}</div></div><div class="card kpi"><h3>Net Cash Movement</h3><div class="big">${KES(s.netCashMovement)}</div></div></div><div class="form-card" style="margin-top:16px"><h3>Owner funding</h3><p>John loan funding: <b>${KES(s.ownerLoanFunding)}</b> · Repayments: <b>${KES(s.ownerLoanRepayment)}</b></p></div><div class="form-card" style="margin-top:16px"><h3>Monthly trend</h3><table><thead><tr><th>Month</th><th>Revenue</th><th>Expenses</th><th>Result</th></tr></thead><tbody>${Object.entries(d.monthly||{}).map(([m,v])=>`<tr><td>${m}</td><td>${KES(v.revenue)}</td><td>${KES(v.expenses)}</td><td>${KES(v.revenue-v.expenses)}</td></tr>`).join('')}</tbody></table></div><div class="form-card" style="margin-top:16px"><h3>John / owner loan balances</h3><table><thead><tr><th>Lender</th><th>Original</th><th>Balance</th><th>Status</th></tr></thead><tbody>${(d.loans||[]).map(l=>`<tr><td>${l.lender||l.debt_name}</td><td>${KES(l.original_principal_kes)}</td><td>${KES(l.current_balance_kes)}</td><td>${l.status}</td></tr>`).join('')}</tbody></table></div></div>`;
}
...
async function loadProductionControls(){
  ...
  try{
    ...
    <div class="form-card" style="margin-top:16px"><div class="section-head"><h3>Anomaly detection</h3><button class="btn ghost sm" id="btn-scan-anomalies">Run scan</button></div>
      <div id="anomaly-list">${(anom.events||[]).filter(x=>x.status==='open').slice(0,20).map(x=>`<div class="narrative" style="margin-top:8px"><b>${x.severity.toUpperCase()}</b> — ${x.message}</div>`).join('')||'<div class="hint">No open anomalies.</div>'}</div>
    </div>`;
    ...
    document.getElementById('btn-executive-snapshot')?.addEventListener('click',async()=>{
      const out=document.getElementById('executive-snapshot-output'); out.innerHTML='<div class="hint">Calculating live financial health…</div>';
      try{
    ...
    document.getElementById('btn-run-scenario')?.addEventListener('click',async()=>{
      const out=document.getElementById('scenario-output'); out.textContent='Running non-posting scenario…';
      try{
        ...
        out.innerHTML=`<b>FORECAST — scenario only</b><br>Monthly revenue: ${fmt(r.monthly_revenue)}<br>Monthly expenses: ${fmt(r.monthly_expenses)}<br>Monthly operating result: ${fmt(r.monthly_operating_result)}<br>Period result: ${fmt(r.period_operating_result)}<div class="hint" style="margin-top:8px">${x.note||''}</div>`;
      }catch(e){out.innerHTML='<div class="err-msg">'+e.message+'</div>';}
    });
...
function viewAutomationCenter(){
  ...
  <div class="grid-3">
    ...
    <div class="stat"><div class="stat-label">Notifications</div><div class="stat-value" id="automation-queue-count">Loading…</div><div class="hint">In-app, email, SMS or webhook delivery.</div></div>
  ...
  <div class="form-card"><div class="section-head"><h3>Automation rules</h3><span class="tag neutral">Branch scoped</span></div><div id="automation-rules"><div class="hint">Loading rules…</div></div></div>
  <div class="form-card" style="margin-top:16px"><div class="section-head"><h3>Recent automation events</h3><span class="tag neutral">Audit trail</span></div><div id="automation-events"><div class="hint">Loading events…</div></div></div>
  <div class="form-card" style="margin-top:16px"><div class="section-head"><h3>Notification queue</h3><span class="tag neutral">Delivery status</span></div><div id="automation-queue"><div class="hint">Loading queue…</div></div></div>
  <div class="form-card" style="margin-top:16px"><div class="section-head"><h3>Automation runs</h3></div><div id="automation-runs"><div class="hint">Loading runs…</div></div></div>`;
}
async function loadAutomationCenter(){
  ...
  try{
    ...
    document.getElementById('btn-run-automation')?.addEventListener('click',async()=>{
      const out=document.getElementById('automation-events');out.innerHTML='<div class="hint">Running automation…</div>';
...
async function loadExecutiveManagement(){
  ...
  out.innerHTML='<div class="hint">Building the executive position from the live financial ledger…</div>';
  try{
    ...
    out.innerHTML=`
      <div class="grid kpi">
        ...
        <div class="card kpi"><div class="kpi-head"><h3>John / Owner Loan</h3><div class="kpi-icon tone-gold">${ic('handshake',16)}</div></div><div class="big">${execMoney(m.owner_loan_balance)}</div><div class="foot">Funding ${execMoney(m.owner_funding)} · Repayment ${execMoney(m.owner_repayments)}</div></div>
...
document.getElementById('btn-complete-statements')?.addEventListener('click',async()=>{try{const from=document.getElementById('stmt-from').value,to=document.getElementById('stmt-to').value;const r=await apiFetch('/api/financial-statements-pro?'+new URLSearchParams({branch_id:state.branchId,from,to}),{headers:JSONH});const d=await r.json();if(!r.ok)throw new Error(d.error||'Statements failed');const p=d.profit_and_loss,b=d.balance_sheet,c=d.cash_flow;document.getElementById('statements-output').innerHTML=`<div class="grid kpi"><div class="card kpi"><h3>Revenue</h3><div class="big">${KES(p.revenue)}</div></div><div class="card kpi"><h3>Expenses</h3><div class="big">${KES(p.expenses)}</div></div><div class="card kpi"><h3>Net Profit</h3><div class="big">${KES(p.net_profit)}</div></div><div class="card kpi"><h3>Net Cash</h3><div class="big">${KES(c.net_cash_movement)}</div></div></div><div class="form-card" style="margin-top:12px"><h3>Profit & Loss</h3><p>Revenue: <b>${KES(p.revenue)}</b> · Expenses: <b>${KES(p.expenses)}</b> · Net Profit: <b>${KES(p.net_profit)}</b></p></div><div class="form-card" style="margin-top:12px"><h3>Balance Sheet</h3><p>Assets: <b>${KES(b.total_assets)}</b> · Liabilities: <b>${KES(b.total_liabilities)}</b> · Equity: <b>${KES(b.total_equity)}</b> · ${b.balanced?'BALANCED':'REVIEW REQUIRED'}</p></div><div class="form-card" style="margin-top:12px"><h3>Cash Flow</h3><p>Operating cash: <b>${KES(c.net_operating_cash)}</b> · Financing cash: <b>${KES(c.net_financing_cash)}</b> · Net movement: <b>${KES(c.net_cash_movement)}</b></p></div>`;}catch(e){alert(e.message)}});document.getElementById('btn-print-statements')?.addEventListener('click',()=>{if(!document.getElementById('statements-output')?.innerHTML){document.getElementById('btn-complete-statements')?.click().then?.(()=>window.print());}else window.print();});
    ...
    document.getElementById('btn-pf-score')?.addEventListener('click',async()=>{try{const d=await pfAction('score');const c=d.compliance;document.getElementById('pf-cycle-output').innerHTML=`<b>Profit First Compliance: ${c.score}/100 — ${c.status}</b><br>${c.coaching_message}<br>Target: ${KES(c.target_kes)} · Transferred: ${KES(c.transferred_kes)} · Variance: ${KES(c.variance_kes)}`;await loadProfitFirstComplete();}catch(e){alert(e.message)}});
...
function wireTab(){
  ...
  async function reconAccountsLoad(){
    ...
    try{ const r=await apiList('/api/financial-accounts',state.branchId); sel.innerHTML='<option value="">Select account…</option>'+(r.accounts||[]).map(a=>`<option value="${a.id}">${a.name} · ${a.kind}</option>`).join(''); }
  ...
  async function reconLoad(id){
    ...
    const exEl=document.getElementById('recon-exceptions'); if(exEl) exEl.innerHTML=`<h4>Exceptions</h4>${(r.exceptions||[]).map(x=>`<div class="form-card" style="padding:10px;margin:6px 0"><b>${x.severity.toUpperCase()}</b> · ${x.description}<span style="float:right">${x.status==='open'?`<button class="btn ghost" data-recon-resolve="${x.id}">Resolve</button>`:'Resolved'}</span></div>`).join('')||'<div class="hint">No exceptions.</div>'}`;
    const audEl=document.getElementById('recon-audit'); if(audEl) audEl.innerHTML=`<details><summary>Reconciliation audit trail</summary>${(r.audit||[]).map(a=>`<div class="hint" style="padding:5px 0;border-bottom:1px solid #eee"><b>${a.event_type}</b> · ${new Date(a.created_at).toLocaleString()}</div>`).join('')}</details>`;
  ...
  async function loadTaxIntelligence(){
    try{
      ...
      const optObs=document.getElementById('tax-period-obligation'); if(optObs)optObs.innerHTML='<option value="">Select obligation…</option>'+obs.filter(o=>o.applicable).map(o=>`<option value="${o.id}">${o.tax_type} · ${o.filing_authority||'Authority'}</option>`).join('');
      const optPeriods=document.getElementById('tax-action-period'); if(optPeriods)optPeriods.innerHTML='<option value="">Select tax period…</option>'+periods.map(x=>`<option value="${x.id}">${x.tax_obligations?.tax_type||'Tax'} · ${x.period_start} → ${x.period_end} · ${x.compliance?.label||''}</option>`).join('');
      const table=document.getElementById('tax-intelligence-table');
      if(table)table.innerHTML=`<table class="data-table"><thead><tr><th>Tax</th><th>Period</th><th>Filing Due</th><th>Payment Due</th><th>Due</th><th>Paid</th><th>Balance</th><th>Filing</th><th>Status</th></tr></thead><tbody>${periods.map(x=>{const bal=Math.max(0,Number(x.amount_due_kes||0)-Number(x.amount_paid_kes||0));const c=x.compliance||{};return `<tr><td>${x.tax_obligations?.tax_type||'—'}</td><td>${x.period_start} → ${x.period_end}</td><td>${x.filing_due_date||'—'}</td><td>${x.payment_due_date||'—'}</td><td>${KES(x.amount_due_kes)}</td><td>${KES(x.amount_paid_kes)}</td><td>${KES(bal)}</td><td>${x.filing_status}</td><td><span class="tag ${c.severity==='critical'?'alert':c.severity==='warning'?'warn':c.severity==='good'?'good':''}">${c.label||'—'}</span>${c.days!==null&&c.days!==undefined?`<div class="hint">${c.days<0?Math.abs(c.days)+' days late':c.days+' days'}</div>`:''}</td></tr>`}).join('')||'<tr><td colspan="9" class="hint">No tax periods have been created yet.</td></tr>'}</tbody></table>`;
...
function renderLogin(errMsg){
        ...
        <div class="login-card">
          ...
          <form id="form-login">
            ...
            <div class="login-row-between">
              <label class="check-row" style="margin:0; text-transform:none; font-weight:500; color:var(--ink-soft);"><input type="checkbox" style="margin:0;"> Remember me</label>
          ...
          <p class="login-foot">Don't have an account? <a id="contact-admin">Contact Admin</a></p>
        </div>
      </div>
    </div>`;
  ...
  document.getElementById('forgot-password').addEventListener('click', (e)=>{
    e.preventDefault();
    alert('Password resets aren\'t self-service yet — contact your Happynet admin to have it reset in Supabase.');
  });
  document.getElementById('contact-admin').addEventListener('click', (e)=>{
    e.preventDefault();
    alert('Accounts are created by the site owner in Supabase — there is no public sign-up. Contact your admin for access.');
  });
